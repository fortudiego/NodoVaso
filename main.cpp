...
static void send_message() {
    ...
    if (RH_T_sensor.getRH(&RH)) {
        printf("\nRelative Humidity is:%6.2f %%\r\n\n",RH);
    } 
    if (RH_T_sensor.getPrevTemp(&temp)) { 
        printf("Ambient Temp. associated to RH reading is:%6.2f degC\r\n\n",temp);
    }
    if (RH_T_sensor.getTemp(&temp)) { 
        printf("Freshly read Ambient Temp. is:%6.2f degC\r\n\n",temp);
    }

    packet_len = sprintf((char *) tx_buffer, "1_%6.2f/%6.2f/ 3.56",RH,temp);

    //packet_len = sprintf((char *) tx_buffer, "Si7021 Sensor Value is  %6.2f umidity,%6.2f temperature ", RH, temp);
